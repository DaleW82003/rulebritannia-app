...
```
┌─────────────────────────────────────────────────────────────────┐
│  Browser (54 × .html pages + vanilla ES2020+ JS, no bundler)    │
...
640+ UK constituencies from the 1997 general election, generated by `scripts/convert-1997-csv.js` from `data/1997_structured.csv`. Each entry has: `id` (slug), `name`, `nation`, `region`, `party` (winning party), `mpType`, `mpName`.
...
- Sessions: DB-backed via `connect-pg-simple`; `sameSite: "lax"`, `secure: true` in production, `httpOnly: true`.
