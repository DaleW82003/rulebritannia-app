...
| Constituencies | `POST /api/admin/constituencies/initialize-scenario`, `POST /api/admin/constituencies/initialize-1997`, `DELETE /api/admin/constituencies/clear` |
...
659 UK constituencies from the 1997 general election, generated by `scripts/convert-scenario-csv.js 1997` from the CSV path declared in `data/scenarios/1997/manifest.json`. Each entry has: `id` (slug), `name`, `nation`, `region`, `party` (winning party), `mpType`, `mpName`.
...
| Script | Purpose |
|---|---|
| `scripts/convert-scenario-csv.js` | Converts a scenario CSV into the manifest-declared `data/scenarios/<key>/constituencies.json` output |
